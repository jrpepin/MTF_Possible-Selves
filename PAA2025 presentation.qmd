...
p1 <- df.mar %>%
  ...
  geom_text_repel(aes(label = Code), data = data_ends.mar, size = 6, nudge_x = 1, nudge_y = -.5) +
  theme_minimal(24) +
  theme(
...
p2 <- df.birth %>%
  ...
  geom_line(linewidth = 2) +
  geom_text_repel(aes(label = Code), data = data_ends.birth, size = 6, nudge_x = 1.5) +
  theme_minimal(24) +
  theme(
    legend.position     = "none",
    legend.title        = element_blank(),
...
p <- df_pp %>%
  ...
  scale_color_manual(values = c("#e74c3c","#18BC9C", "#3498DB")) +
  labs( x        = " ", 
        y        = " ")

[p](https://github.com/jrpepin/MTF_Possible-Selves/blob/main/output/figs/averages.png)
```
